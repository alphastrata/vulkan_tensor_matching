...
fn main() {
    ...
    let extracted_template =
        extract_patch(&target, extract_x, extract_y, extract_size, extract_size);
    println!(
        "Extracted template: {}x{}",
        extracted_template.width, extracted_template.height
    );

    // Save the extracted template for verification
    let extracted_img = image_data_to_rgb_image(&extracted_template);
    extracted_img
        .save("extracted_template.png")
        .expect("Failed to save extracted template");
...
fn extract_patch(source: &ImageData, x: u32, y: u32, width: u32, height: u32) -> ImageData {
