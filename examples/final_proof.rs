...
fn main() {
    ...
    let matcher =
        vulkan_tensor_matching::VulkanTensorMatcher::new().expect("Failed to create matcher");
        ...
        .enumerate()
    {
        ...
        println!(
            "  GT patch vs Template correlation: {:.3} {}",
            gt_corr,
            if gt_valid { "✓" } else { "✗" }
        );
        ...
        let matches = matcher
            .match_template(&img, &template, 0.3, 1)
        ...
        for x in gt_x..gt_x + gt_w {
            if x < viz.width() {
                viz.put_pixel(x, gt_y, Rgb([128, 0, 128]));
            }
            if x < viz.width() {
                viz.put_pixel(x, gt_y + gt_h - 1, Rgb([128, 0, 128]));
            }
        }
        for y in gt_y..gt_y + gt_h {
            if y < viz.height() {
                viz.put_pixel(gt_x, y, Rgb([128, 0, 128]));
            }
            if y < viz.height() {
                viz.put_pixel(gt_x + gt_w - 1, y, Rgb([128, 0, 128]));
            }
        }
        ...
        for x in det_x..det_x + template.width {
            if x < viz.width() {
                viz.put_pixel(x, det_y, match_color);
            }
            if x < viz.width() {
                viz.put_pixel(x, det_y + template.height - 1, match_color);
            }
        }
        for y in det_y..det_y + template.height {
            if y < viz.height() {
                viz.put_pixel(det_x, y, match_color);
            }
            if y < viz.height() {
                viz.put_pixel(det_x + template.width - 1, y, match_color);
            }
        }
        ...
        results.push(CaseResult {
            ...
            vulkan_x: best.x,
            vulkan_y: best.y,
            vulkan_rot: best.rotation_angle,
            duration_s,
        });
    ...
    for r in &results {
        ...
        md.push_str(&format!(
            ...
            if r.gt_valid {
                ...
                "✗ No - template doesn't match at this location!"
            }
        ));
        ...
        md.push_str(&format!(
            "![Proof](proof_output/case_{:03}_proof.png)\n\n",
            r.case_idx
        ));
        md.push_str(&format!(
            "![Template](proof_output/case_{:03}_template.png)\n\n",
            r.case_idx
        ));
        md.push_str("---\n\n");
    }
