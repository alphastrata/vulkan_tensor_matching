        ...
        if let Ok(matches) = matcher.match_template(img, &template, 0.5, 1) {
            if let Some(best) = matches.first() {
                ...
                if distance < VERIFICATION_THRESHOLD && best.correlation > 0.7 {
                    println!(
                        "  Verified location: ({}, {}) corr={:.3} dist={:.1}px",
                        x, y, best.correlation, distance
                    );
...
fn process_image(
    ...
    for i in 0..15 {
        ...
        if gt_cy + i < viz.height() {
            viz.put_pixel(gt_cx, gt_cy + i, Rgb([128, 0, 128]));
        }
        if gt_cy - i < viz.height() {
            viz.put_pixel(gt_cx, gt_cy - i, Rgb([128, 0, 128]));
        }
...
fn main() {
    ...
    println!(
        ...
        success,
        results.len(),
        100.0 * success as f32 / results.len() as f32
    );
