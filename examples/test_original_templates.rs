...
fn main() {
        ...
        .enumerate()
    {
        ...
        println!("  GT: ({}, {}) angle={:.1}°", gt_x, gt_y, gt_angle.to_degrees());
        ...
        let matches = matcher.match_template(&img, &template, 0.3, 3).expect("Matching failed");
        ...
        println!("  Match: ({}, {}) corr={:.3} dist={:.1}px ({:.1}s)", 
    ...
    for (idx, success, dist, corr) in &results {
        ...
        println!("  Case {}: {} dist={:.1}px corr={:.3}", idx, status, dist, corr);
    }
}
