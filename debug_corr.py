...
for name, tl, wrong in [
    ...
    print(f"\n{name}:")
    print(
        f"  GT   ({tl[0]},{tl[1]}): var={t.var():.6f}, std={t.std():.4f}, mean={t.mean():.4f}"
    )
    print(
        f"  Wrong({wrong[0]},{wrong[1]}): var={w.var():.8f}, std={w.std():.6f}, mean={w.mean():.4f}"
    )

    # Check if wrong patch is almost uniform
    print(
        f"  Wrong patch min={w.min():.4f} max={w.max():.4f} range={w.max() - w.min():.6f}"
    )
