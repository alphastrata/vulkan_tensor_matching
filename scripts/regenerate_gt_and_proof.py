...
def process_image(img_path, case_idx, matcher):
    ...
    print(f"  Template location: top-left=({tmpl_x}, {tmpl_y}), center=({tmpl_center_x}, {tmpl_center_y})")
    ...
    if vulkan_matches:
        ...
        print(f"  Vulkan: ({best.x}, {best.y}) corr={best.correlation:.3f} ({vulkan_duration:.1f}ms)")
    
    # Match with OpenCV
    opencv_result = opencv_match(img_np, tmpl_np)
    if opencv_result:
        print(f"  OpenCV: ({opencv_result['x']:.1f}, {opencv_result['y']:.1f}) corr={opencv_result['corr']:.3f}")
    
    # Compute distances
    if vulkan_result:
        vulkan_dist = (((vulkan_result["x"] - tmpl_center_x)**2 + (vulkan_result["y"] - tmpl_center_y)**2)**0.5)
        print(f"  Vulkan distance to GT: {vulkan_dist:.1f}px")

    if opencv_result:
        opencv_dist = (((opencv_result["x"] - tmpl_center_x)**2 + (opencv_result["y"] - tmpl_center_y)**2)**0.5)
    ...
    # Ground truth (purple)
    draw_match(rgb_img, tmpl_center_x, tmpl_center_y, TEMPLATE_SIZE, TEMPLATE_SIZE,
               (128, 0, 128), "GT", is_center=True)
    
    # Vulkan match (green)
    if vulkan_result:
        color = (0, 255, 0) if vulkan_dist < 15 else (255, 165, 0)
        draw_match(rgb_img, vulkan_result["x"], vulkan_result["y"], TEMPLATE_SIZE, TEMPLATE_SIZE,
                   color, f"Vulkan: {vulkan_result['corr']:.2f}", is_center=True)
    
    # OpenCV match (orange)
    if opencv_result:
        draw_match(rgb_img, opencv_result["x"], opencv_result["y"], TEMPLATE_SIZE, TEMPLATE_SIZE,
                   (255, 165, 0), f"OpenCV: {opencv_result['corr']:.2f}", is_center=True)
...
def generate_proof_md(results):
    ...
    for r in results:
        status = "✓" if (r["vulkan_distance"] < 15 if r["vulkan_distance"] else False) else "⚠️"
        vulkan_corr = f"{r['vulkan_result']['corr']:.3f}" if r["vulkan_result"] else "N/A"
        opencv_corr = f"{r['opencv_result']['corr']:.3f}" if r["opencv_result"] else "N/A"
        ...
        md.append(f"| {len(md)-5} | {Path(r['image_path']).name} | {vulkan_corr} | {opencv_corr} | {vulkan_dist} | {opencv_dist} | {status} |")
    ...
    for i, r in enumerate(results):
        ...
        md.append("")
        md.append(f"**Template Location (Ground Truth)**: top-left=({r['expected_matches'][0]['x']}, {r['expected_matches'][0]['y']})")
        md.append("")
        
        if r["vulkan_result"]:
            md.append(f"**Vulkan TTM Result**:")
            md.append(f"- Position: ({r['vulkan_result']['x']:.1f}, {r['vulkan_result']['y']:.1f})")
            md.append(f"- Correlation: {r['vulkan_result']['corr']:.3f}")
            md.append(f"- Rotation: {r['vulkan_result']['rotation']:.1f} rad ({np.degrees(r['vulkan_result']['rotation']):.1f}°)")
        ...
        if r["opencv_result"]:
            ...
            md.append(f"- Position: ({r['opencv_result']['x']:.1f}, {r['opencv_result']['y']:.1f})")
...
def main():
    ...
    vulkan_success = sum(1 for r in results if r["vulkan_distance"] and r["vulkan_distance"] < 15)
    opencv_success = sum(1 for r in results if r["opencv_distance"] and r["opencv_distance"] < 15)
