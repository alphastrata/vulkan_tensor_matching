...
def process_image(img_path, case_idx, matcher):
    ...
    for attempt in range(3):
        if attempt > 0:
            ...
            offset = attempt * 50
            loc_result = find_distinctive_location(
                img_np, TEMPLATE_SIZE, margin=50 + offset
            )
        ...
        tmpl_x, tmpl_y = loc
        print(
            f"  Attempt {attempt + 1}: Location ({tmpl_x}, {tmpl_y}) variance={variance:.4f}"
        )
        ...
        # Verify match
        best_match, dist = verify_match(
            matcher, img_data, template, tmpl_x, tmpl_y, TEMPLATE_SIZE
        )
        ...
        print(
            f"    Match: ({best_match.x}, {best_match.y}) corr={best_match.correlation:.3f} dist={dist:.1f}px"
        )
    ...
    draw.line(
        [(gt_cx - 15, gt_cy), (gt_cx + 15, gt_cy)], fill=(128, 0, 128, 255), width=2
    )
    draw.line(
        [(gt_cx, gt_cy - 15), (gt_cx, gt_cy + 15)], fill=(128, 0, 128, 255), width=2
    )
    ...
    draw.rectangle(
        [det_x, det_y, det_x + TEMPLATE_SIZE, det_y + TEMPLATE_SIZE],
        outline=(0, 255, 0, 255),
        width=2,
    )
    draw.text(
...
def generate_proof_md(results):
    ...
    for i, r in enumerate(results):
        ...
        md.append(
            f"| {i} | {Path(r['image_path']).name} | {r['vulkan_result']['corr']:.3f} | {r['distance']:.1f}px | {status} |"
        )
...
def main():
    ...
    print(
        f"Success rate (< 10px): {success}/{len(results)} ({100 * success / len(results):.0f}%)"
    )
