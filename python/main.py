...
def main():
    ...
    print(
        f"   Max value: {extremes['max']['value']:.4f} at ({extremes['max']['x']}, {extremes['max']['y']})"
    )
    print(
        f"   Min value: {extremes['min']['value']:.4f} at ({extremes['min']['x']}, {extremes['min']['y']})"
    )
    ...
    result = match_template_cpu(
        img_data, tmpl_data, MatchTemplateMethod.cross_correlation_normalized()
    )
    ...
    try:
        ...
        matches = matcher.match_template(
            img_data, tmpl_data, correlation_threshold=0.5, max_matches=5
        )
