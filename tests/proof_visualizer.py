...
def extract_patch(img_data, x, y, w, h):
    ...
    img_array = np.array(img_data.data, dtype=np.float32).reshape(img_data.height, img_data.width)
...
def process_image(img_path, case_idx, matcher):
    ...
    if vulkan_matches:
        ...
        print(f"  Vulkan: ({vulkan_result['x']},{vulkan_result['y']}) corr={vulkan_result['corr']:.3f} dist={vulkan_dist:.1f}px ({vulkan_duration:.2f}s)")
    else:
    ...
    # Ground truth (purple)
    draw_match(rgb_img, gt_cx, gt_cy, TEMPLATE_SIZE, TEMPLATE_SIZE, (128, 0, 128), "GT", is_center=True)
    
    # NumPy (orange)
    if numpy_result:
        draw_match(rgb_img, numpy_result["x"], numpy_result["y"], TEMPLATE_SIZE, TEMPLATE_SIZE, 
                   (255, 165, 0), f"NumPy: {numpy_result['corr']:.2f}", is_center=True)
...
def generate_proof_md(results):
    ...
    md = ["# Vulkan Tensorial Template Matching - Proof Results",
          "",
    ...
    for i, r in enumerate(results):
        vulkan_status = "✓" if r["vulkan_distance"] is not None and r["vulkan_distance"] < 50 else "✗"
        
        md.append(f"| {i} | {Path(r['image_path']).name} | N/A | {r['vulkan_result']['corr']:.3f} | N/A | {r['vulkan_distance']:.1f}px | {vulkan_status} |")
...
def main():
    ...
    vulkan_pass = sum(1 for r in results if r["vulkan_distance"] is not None and r["vulkan_distance"] < 50)
