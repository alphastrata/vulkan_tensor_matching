...
def main():
    ...
    for i, val in enumerate(patch):
        tmpl_img.putpixel((i % tmpl_size, i // tmpl_size), int(max(0, min(255, val * 255))))
    ...
    for i, m in enumerate(matches):
        ...
        print(f"  Match {i+1}: ({m.x}, {m.y}) corr={m.correlation:.3f} dist={dist:.1f}px")
    ...
    if matches:
        ...
        for i, val in enumerate(det_patch):
            det_img.putpixel((i % tmpl_size, i // tmpl_size), int(max(0, min(255, val * 255))))
