...
def draw_comparison(title, template, at_detected, at_gt, output_path):
    ...
    draw.text((ts[0] + 10, 35), f"Detected\nCorr: {title.split(',')[0] if ',' in title else 'N/A'}", fill="green")
    if gs:
...
def main():
    ...
    for case_idx, case in enumerate(answers[:10]):  # Test first 10 cases
        ...
        print(f"  Claimed GT: top-left=({gt_x}, {gt_y}), center=({gt_center_x}, {gt_center_y})")
        ...
        best = matches[0]
        print(f"  Best Match: ({best.x}, {best.y}) corr={best.correlation:.3f} rot={best.rotation_angle:.1f}rad")
        print(f"  Distance from GT center: {((best.x - gt_center_x)**2 + (best.y - gt_center_y)**2)**0.5:.1f}px")
        ...
        result = {
            ...
            "distance_px": ((best.x - gt_center_x)**2 + (best.y - gt_center_y)**2)**0.5,
