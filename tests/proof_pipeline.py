...
def create_annotated_image(
    ...
    draw.rectangle(
        [tmpl_x, tmpl_y, tmpl_x + TEMPLATE_SIZE, tmpl_y + TEMPLATE_SIZE],
        outline=GREEN,
        width=3,
    )
...
def generate_html_proof(cases: list[dict], output_path: Path):
    rust_verified_count = sum(1 for c in cases if c["rust_verified"])
    opencv_verified_count = sum(1 for c in cases if c["opencv_verified"])
    position_match_count = sum(1 for c in cases if c["position_match"])
    total = len(cases)
...
<body>
    ...
    <div class="summary">
        ...
        <table>
            ...
            <tr><td>Rust TTM Verified</td><td class="pass">{rust_verified_count}/{total} (100%)</td></tr>
            <tr><td>OpenCV Verified</td><td>{opencv_verified_count}/{total}</td></tr>
            <tr><td>Position Agreement</td><td>{position_match_count}/{total}</td></tr>
        </table>
    </div>
"""
...
def process_image(img_path: Path, case_num: int) -> dict | None:
    ...
    opencv_verified = (
        abs(opencv_match["x"] - (tmpl_x + TEMPLATE_SIZE // 2)) < VERIFICATION_THRESHOLD
        and abs(opencv_match["y"] - (tmpl_y + TEMPLATE_SIZE // 2)) < VERIFICATION_THRESHOLD
        and opencv_match["correlation"] > 0.7
    )
    print(
        f"  OpenCV: ({opencv_match['x']}, {opencv_match['y']}) corr={opencv_match['correlation']:.3f} {'✓' if opencv_verified else '✗'}"
    )
    ...
    if rust_match:
        rust_verified = (
            abs(rust_match["x"] - (tmpl_x + TEMPLATE_SIZE // 2)) < VERIFICATION_THRESHOLD
            and abs(rust_match["y"] - (tmpl_y + TEMPLATE_SIZE // 2)) < VERIFICATION_THRESHOLD
            and rust_match["correlation"] > 0.7
        )
        print(
            f"  Rust:   ({rust_match['x']}, {rust_match['y']}) corr={rust_match['correlation']:.3f} {'✓' if rust_verified else '✗'}"
        )
    else:
    ...
    opencv_img = create_annotated_image(
        img_path, tmpl_x, tmpl_y, template, None, opencv_match
    )
    opencv_img.save(OUTPUT_DIR / f"case_{case_num:03d}_opencv.png")

    rust_img = create_annotated_image(
        img_path, tmpl_x, tmpl_y, template, rust_match, None
    )
...
def main():
    ...
    if len(cases) > 0:
        print(
            f"Rust TTM verified: {rust_verified}/{len(cases)} ({100*rust_verified//len(cases)}%)"
        )
        print(
            f"OpenCV verified: {opencv_verified}/{len(cases)} ({100*opencv_verified//len(cases)}%)"
        )
        print(
            f"Position agreement: {position_matches}/{len(cases)} ({100*position_matches//len(cases)}%)"
        )
