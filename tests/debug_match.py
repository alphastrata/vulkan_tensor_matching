...
def main():
    ...
    for i, m in enumerate(matches):
        ...
        print(f"  Match {i+1}: ({m.x}, {m.y}) corr={m.correlation:.3f} rot={m.rotation_angle:.2f}rad ({deg:.1f}°) dist={dist:.1f}px")
    
    draw = ImageDraw.Draw(lenna_pil)
    draw.rectangle([tmpl_x, tmpl_y, tmpl_x + tmpl_size, tmpl_y + tmpl_size], outline="purple", width=3)
    ...
    for i, m in enumerate(matches[:3]):
        ...
        draw.rectangle([det_x, det_y, det_x + tmpl_size, det_y + tmpl_size], outline=color, width=2)
    ...
    for y in range(0, lenna_np.shape[0] - tmpl_size, step):
        for x in range(0, lenna_np.shape[1] - tmpl_size, step):
            ...
            if patch_std < 1e-6:
                continue
            patch_norm = (patch - patch_mean) / (patch_std * np.sqrt(tmpl_size * tmpl_size))
    ...
    if best_loc:
        ...
        dist = math.sqrt(dx*dx + dy*dy)
        print(f"Best match: ({best_loc[0]}, {best_loc[1]}) center=({best_cx}, {best_cy}) corr={best_corr:.3f}")
