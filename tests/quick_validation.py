...
def main():
    ...
    print(f"Extracted {extract_size}x{extract_size} template from ({extract_x}, {extract_y})")
    ...
    for i, m in enumerate(matches[:3]):
        print(f"  Match {i+1}: ({m.x}, {m.y}) corr={m.correlation:.3f} rot={m.rotation_angle:.1f}rad")
    ...
    draw_match(lenna_rgb, expected_cx, expected_cy, extract_size, extract_size, 0, 
               (128, 0, 128), "Expected", is_center=True)
    
    # Draw detected matches (green)
    for i, m in enumerate(matches[:3]):
        color = (0, 255, 0) if m.correlation > 0.9 else (255, 255, 0)
        draw_match(lenna_rgb, m.x, m.y, extract_size, extract_size, m.rotation_angle,
                   color, f"Detected #{i+1}", is_center=True)
    ...
    print(f"Testing with {extract_size_2}x{extract_size_2} template from ({extract_x_2}, {extract_y_2})")
    ...
    if template_path.exists():
        ...
        for i, m in enumerate(matches[:5]):
            ...
            draw_match(lenna_rgb2, m.x, m.y, test_template.width, test_template.height, 
                       m.rotation_angle, color, f"#{i+1}: {m.correlation:.2f}", is_center=True)
